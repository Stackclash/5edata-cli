...
describe('Collect', () => {
  ...
  describe('plugin discovery', () => {
    it('should identify ttrpg-data-forge plugins', () => {
      ...
      const filteredPlugins = [...plugins.keys()].filter(p => 
        p.includes('ttrpg-data-forge') || p.includes('forge-plugin')
      )
  ...
  describe('source parsing', () => {
    ...
    it('should handle empty strings in source list', () => {
      ...
      const sources = sourcesArg.split(',').map(s => s.trim().toLowerCase()).filter(s => s !== '')
