...
describe('Generate', () => {
  describe('sources argument', () => {
    it('should handle sources argument parsing', async () => {
      const { command, mocks } = createMockedCommand(Generate, ['-s', '5etools,dndbeyond'])

      await command.run()

      const foundSourcesLogCall = mocks.log
        .getCalls()
        .find((call: SinonSpyCall) => call.firstArg.includes('Sources to generate from'))
    ...
    it('should filter sources correctly', async () => {
      const { command, mocks } = createMockedCommand(Generate, ['-s', '5etools'])
    ...
    //   const { command, mocks } = createMockedCommand(Generate, ['-s', ''])
    ...
    //   const { command, mocks } = createMockedCommand(Generate, ['-s', 'invalidsource'])
    //   let errorLogCall: SinonSpyCall | undefined
  ...
  describe('plugin discovery', () => {
    it('should identify forge-plugin plugins', async () => {
      const mockConfig = createMockConfig(
        new Map<string, { commandIDs: string[] }>([
          ['forge-plugin-test1', { commandIDs: ['generate', '5etools'] }],
          ['some-other-plugin', { commandIDs: ['other:command'] }],
          ['test-forge-plugin-test2', { commandIDs: ['data:generate', 'dndbeyond'] }],
        ]),
      )
      const { command, mocks } = createMockedCommand(Generate, [], mockConfig)
    ...
    //   const mockConfig = createMockConfig(
    //     new Map<string, { commandIDs: string[] }>([
    //       ['another-plugin', { commandIDs: ['data:generate', 'dndbeyond'] }],
    //       ['some-other-plugin', { commandIDs: ['other:command'] }],
    //     ]),
    //   )
    //   const { command, mocks } = createMockedCommand(Generate, [], mockConfig)
  ...
  describe('source name extraction', () => {
    it('should extract source name from forge-plugin plugins', async () => {
      const mockConfig = createMockConfig(
        new Map<string, { commandIDs: string[] }>([
          ['complex-forge-plugin-name-example', { commandIDs: ['generate'] }],
          ['forge-plugin-5etools', { commandIDs: ['generate', '5etools'] }],
          ['forge-plugin-dndbeyond', { commandIDs: ['data:generate', 'dndbeyond'] }],
          ['some-other-plugin', { commandIDs: ['other:command'] }],
        ]),
      )
      const { command, mocks } = createMockedCommand(Generate, [], mockConfig)
  ...
  describe('command ID discovery', () => {
    it('should find generate command', async () => {
      ...
      expect(mocks.runCommand.firstCall.args[0]).to.equal('generate')
      expect(mocks.runCommand.secondCall.args[0]).to.equal('data:generate')
    })

    it('should warn if plugin has no generate command', async () => {
      const mockConfig = createMockConfig(
        new Map<string, { commandIDs: string[] }>([
      ...
      expect(mocks.runCommand.firstCall.args[0]).to.equal('data:generate')
      const warnLogCall = mocks.warn
        .getCalls()
        .find((call: SinonSpyCall) => call.firstArg.includes('does not have a generate command'))
